...
N=10000 # Must be divisible by 20

null_rao_mat = matrix(NA, ncol=nrow(sampXmorph), nrow=N)
colnames(null_rao_mat) = rownames(sampXmorph)

this_run = paste('withinreg', sfact, use_null_alg, sep='-')


# Read in previously completed null matrix
null_rao_mat = read.csv(paste('null_rao-', this_run, '.csv', sep=''), check.names=F)

unfinished_regions = 'Mountains' #unique(plot_data$Ecoregion)
start_j = 6 #1


for(this_reg in unfinished_regions){
	# Define community matrix for this plot
	these_plots = rownames(subset(plot_data, Ecoregion==this_reg))
	these_samps = subset(samp_data, PlotID%in%these_plots)$SampID
	...
	for(j in start_j:20){
		nulls = simulate(nullmodel(x, use_null_alg), N/20)

		# Calculate rao's Q on each matrix
		null_raos = sapply(1:(N/20), function(i){
			null_scaled = as.matrix(nulls[,,i]/rowSums(nulls[,,i]))
			calc_rao(null_scaled, use_dmat)
		})

		null_rao_mat[(((j-1)*(N/20))+1):(j*N/20),these_samps] = null_raos
	
		# Save current progress
		write.csv(null_rao_mat, paste('null_rao-', this_run, '.csv', sep=''), row.names=F)
		print(paste('Done with',this_reg,((j-1)*(N/20))+1,':',j*N/20))
